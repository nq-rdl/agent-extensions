#!/usr/bin/env bash
# sql-code plugin smoke E2E (#131 / #126 #127 #128 #130).
...
# installs sql-code, then drives the INSTALLED helper and hooks in a scratch project. With
# credentials available it also runs the first-run acceptance test (`claude -p '/sql-code:setup
...
jq -e '.plugins[]|select(.name=="sql-code")' "$MP" >/dev/null 2>&1 && pass "A: sql-code in marketplace.json" || bad "A: sql-code missing from marketplace.json"
[ -f registry/bundles/sql-code.yaml ] && pass "A: bundle yaml" || bad "A: bundle yaml missing"
for leaf in setup bootstrap analyse explain guardrails map draft validate; do
  [ -f "plugins/sql-code/skills/$leaf/SKILL.md" ] && pass "B: plugin skill $leaf synced" || bad "B: plugin skill $leaf missing"
  grep -q '^name:' "plugins/sql-code/skills/$leaf/SKILL.md" 2>/dev/null && bad "B: $leaf copy still carries name:" || pass "B: $leaf copy has no name: (labels as sql-code:$leaf)"
done
for f in scripts/sqlreview.sh scripts/sqlreview-lib.sh scripts/sqlreview-check.jq scripts/sqlreview-render.jq \
  ...
  [ -f "plugins/sql-code/skills/setup/$f" ] && pass "C: setup/$f shipped" || bad "C: setup/$f missing from plugin"
done
for h in sql-code-guard.sh sql-code-preflight.sh; do
  cmp -s "hooks/$h" "plugins/sql-code/hooks/$h" && pass "D: $h copy identical" || bad "D: $h copy differs from hooks/"
done
jq -e '.hooks.PreToolUse[0].matcher == "Write|Edit" and (.hooks.SessionStart|length) == 1' plugins/sql-code/hooks/hooks.json >/dev/null 2>&1 && pass "D: hooks.json wiring" || bad "D: hooks.json wiring"
...
drive_hooks() { # <plugin root> <label>
  ...
  out="$(printf '{"tool_name":"Write","tool_input":{"file_path":"%s/.sqlreview/reviews/x/review.json","content":"{\\"kind\\":\\"review\\",\\"revision\\":1,\\"assumptions\\":[{\\"id\\":\\"A1\\",\\"text\\":\\"x\\",\\"status\\":\\"pending\\"}],\\"limitations\\":[]}"},"cwd":"%s"}' "$tmp" "$tmp" \
        | CLAUDE_PLUGIN_ROOT="$P" bash "$P/hooks/sql-code-guard.sh")"
  ...
  out="$(printf '{"cwd":"%s"}' "$tmp" | CLAUDE_PLUGIN_ROOT="$P" bash "$P/hooks/sql-code-preflight.sh")"
...
drive_helper "$root/skills/sql-code-setup/scripts/sqlreview.sh" "E: canonical helper"
drive_hooks "$root/plugins/sql-code" "E: repo plugin copy"
...
if [ "${1:-}" = "--live" ]; then
  ...
  if ! command -v claude >/dev/null; then
    ...
    trap 'rm -rf "$tmp"' EXIT
    if claude plugin validate "$ws/plugins/sql-code" >"$tmp/validate.out" 2>&1; then pass "live: plugin validate"; else bad "live: plugin validate failed"; tail -3 "$tmp/validate.out"; fi
    ...
    if claude plugin install sql-code@rdl-agent-extensions >"$tmp/install.out" 2>&1; then pass "live: install sql-code@rdl-agent-extensions"; else bad "live: install failed"; tail -3 "$tmp/install.out"; fi
    if list="$(claude plugin list 2>&1)"; then
      printf '%s\n' "$list" | grep -Eiq '(^|[^[:alnum:]_])sql-code([^[:alnum:]_]|$)' && pass "live: sql-code listed as installed" || { bad "live: sql-code not in plugin list"; printf '%s\n' "$list" | tail -5; }
    else bad "live: plugin list failed"; fi
    # The installed copy (the cache Claude Code actually runs) must be self-contained.
    inst="$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/plugins/cache" -maxdepth 4 -type d -path '*rdl-agent-extensions/sql-code/*' 2>/dev/null | head -1)"
    if [ -n "$inst" ] && [ -f "$inst/skills/setup/scripts/sqlreview.sh" ]; then
      ...
      bad "live: installed sql-code copy not found under the plugin cache"
    fi
    ...
    if [ -f "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.credentials.json" ] || [ -n "${ANTHROPIC_API_KEY:-}" ]; then
      ...
      if ( cd "$proj" && timeout 600 claude -p '/sql-code:setup --default --yes' --permission-mode bypassPermissions >"$tmp/setup.out" 2>&1 ) && [ -f "$proj/.sqlreview/config.json" ]; then
        pass "live: /sql-code:setup --default --yes created .sqlreview/config.json"
        ...
        bad "live: /sql-code:setup did not create .sqlreview/config.json"; tail -15 "$tmp/setup.out"
      fi
    else
