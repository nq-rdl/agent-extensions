#!/usr/bin/env bash
# sql-review plugin smoke E2E (#131 / #126 #127 #128 #130).
...
# installs sql-review, then drives the INSTALLED helper and hooks in a scratch project. With
# credentials available it also runs the first-run acceptance test (`claude -p '/sql-review:setup
...
jq -e '.plugins[]|select(.name=="sql-review")' "$MP" >/dev/null 2>&1 && pass "A: sql-review in marketplace.json" || bad "A: sql-review missing from marketplace.json"
[ -f registry/bundles/sql-review.yaml ] && pass "A: bundle yaml" || bad "A: bundle yaml missing"
for leaf in setup bootstrap analyse explain; do
  [ -f "plugins/sql-review/skills/$leaf/SKILL.md" ] && pass "B: plugin skill $leaf synced" || bad "B: plugin skill $leaf missing"
  grep -q '^name:' "plugins/sql-review/skills/$leaf/SKILL.md" 2>/dev/null && bad "B: $leaf copy still carries name:" || pass "B: $leaf copy has no name: (labels as sql-review:$leaf)"
done
for f in scripts/sqlreview.sh scripts/sqlreview-lib.sh scripts/sqlreview-check.jq scripts/sqlreview-render.jq \
  ...
  [ -f "plugins/sql-review/skills/setup/$f" ] && pass "C: setup/$f shipped" || bad "C: setup/$f missing from plugin"
done
for h in sql-review-guard.sh sql-review-preflight.sh; do
  cmp -s "hooks/$h" "plugins/sql-review/scripts/$h" && pass "D: $h copy identical" || bad "D: $h copy differs from hooks/"
done
jq -e '.hooks.PreToolUse[0].matcher == "Write|Edit" and (.hooks.SessionStart|length) == 1' plugins/sql-review/hooks/hooks.json >/dev/null 2>&1 && pass "D: hooks.json wiring" || bad "D: hooks.json wiring"
...
drive_hooks() { # <plugin root> <label>
  ...
  out="$(printf '{"tool_name":"Write","tool_input":{"file_path":"%s/.sqlreview/reviews/x/review.json","content":"{\\"kind\\":\\"review\\",\\"revision\\":1,\\"assumptions\\":[{\\"id\\":\\"A1\\",\\"text\\":\\"x\\",\\"status\\":\\"pending\\"}],\\"limitations\\":[]}"},"cwd":"%s"}' "$tmp" "$tmp" \
        | CLAUDE_PLUGIN_ROOT="$P" bash "$P/scripts/sql-review-guard.sh")"
  ...
  out="$(printf '{"cwd":"%s"}' "$tmp" | CLAUDE_PLUGIN_ROOT="$P" bash "$P/scripts/sql-review-preflight.sh")"
...
drive_helper "$root/skills/sql-review-setup/scripts/sqlreview.sh" "E: canonical helper"
drive_hooks "$root/plugins/sql-review" "E: repo plugin copy"
...
if [ "${1:-}" = "--live" ]; then
  ...
  if ! command -v claude >/dev/null; then
    ...
    trap 'rm -rf "$tmp"' EXIT
    if claude plugin validate "$ws/plugins/sql-review" >"$tmp/validate.out" 2>&1; then pass "live: plugin validate"; else bad "live: plugin validate failed"; tail -3 "$tmp/validate.out"; fi
    ...
    if claude plugin install sql-review@rdl-agent-extensions >"$tmp/install.out" 2>&1; then pass "live: install sql-review@rdl-agent-extensions"; else bad "live: install failed"; tail -3 "$tmp/install.out"; fi
    if list="$(claude plugin list 2>&1)"; then
      printf '%s\n' "$list" | grep -Eiq '(^|[^[:alnum:]_])sql-review([^[:alnum:]_]|$)' && pass "live: sql-review listed as installed" || { bad "live: sql-review not in plugin list"; printf '%s\n' "$list" | tail -5; }
    else bad "live: plugin list failed"; fi
    # The installed copy (the cache Claude Code actually runs) must be self-contained.
    inst="$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/plugins/cache" -maxdepth 4 -type d -path '*rdl-agent-extensions/sql-review/*' 2>/dev/null | head -1)"
    if [ -n "$inst" ] && [ -f "$inst/skills/setup/scripts/sqlreview.sh" ]; then
      ...
      bad "live: installed sql-review copy not found under the plugin cache"
    fi
    ...
    if [ -f "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.credentials.json" ] || [ -n "${ANTHROPIC_API_KEY:-}" ]; then
      ...
      if ( cd "$proj" && timeout 600 claude -p '/sql-review:setup --default --yes' --permission-mode bypassPermissions >"$tmp/setup.out" 2>&1 ) && [ -f "$proj/.sqlreview/config.json" ]; then
        pass "live: /sql-review:setup --default --yes created .sqlreview/config.json"
        ...
        bad "live: /sql-review:setup did not create .sqlreview/config.json"; tail -15 "$tmp/setup.out"
      fi
    else
