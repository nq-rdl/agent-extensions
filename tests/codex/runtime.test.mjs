...
test("task resolves GPT-5.6 model aliases (sol/terra/luna) to full ids", () => {
  for (const [alias, full] of [
    ["sol", "gpt-5.6-sol"],
    ["terra", "gpt-5.6-terra"],
    ["luna", "gpt-5.6-luna"]
  ]) {
    ...
    assert.equal(result.status, 0, result.stderr);
    const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
    assert.equal(fakeState.lastTurnStart.model, full, `alias ${alias}`);
  }
});

test("task warns on an unrecognized reasoning effort but passes it through", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const statePath = path.join(binDir, "fake-codex-state.json");
  installFakeCodex(binDir);
  ...
  const result = run("node", [SCRIPT, "task", "--effort", "bogus", "diagnose the failing test"], {
    cwd: repo,
    env: buildEnv(binDir)
  });

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stderr, /unrecognized reasoning effort/i);
  const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.equal(fakeState.lastTurnStart.effort, "bogus");
});

test("task accepts the max and ultra efforts without warning", () => {
  for (const effort of ["max", "ultra"]) {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.doesNotMatch(result.stderr, /unrecognized reasoning effort/i);
    const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
    assert.equal(fakeState.lastTurnStart.effort, effort, `effort ${effort}`);
  }
});
