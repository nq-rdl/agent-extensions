"""Behavioural tests for the sql-code plugin's hooks.

hooks/sql-code-guard.sh    PreToolUse (Write|Edit): the assumption/limitation confirmation
                             invariant on review/scope JSON, rendered-markdown protection, config ask.
hooks/sql-code-preflight.sh SessionStart: one factual context line about the project's state.

Both are generated by sync-plugins.sh to plugins/sql-code/hooks/ (a test asserts byte-identical copies). Each
...
GUARD = REPO / "hooks" / "sql-code-guard.sh"
PREFLIGHT = REPO / "hooks" / "sql-code-preflight.sh"
...
class GuardHook(unittest.TestCase):
    ...
    def test_config_asks_and_templates_pass(self):
        ...
        for ev in (write_event(cfg, "{}", self.root), edit_event(cfg, self.root)):
            with self.subTest(ev["tool_name"]):
                ...
                self.assertIn("/sql-code:setup", d["permissionDecisionReason"])
    ...
    def test_plugin_copy_resolves_checker_without_plugin_root(self):
        ...
        r = run_hook(PLUGIN / "hooks" / "sql-code-guard.sh",
...
class PreflightHook(unittest.TestCase):
    def test_initialised_project_reports_counts_and_stale_slugs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("/sql-code:analyse", ctx)
    ...
    def test_uninitialised_project_with_sql_files_hints_setup(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("/sql-code:setup", ctx)
    ...
    def test_missing_helper_or_cwd_is_a_noop(self):
        with tempfile.TemporaryDirectory() as tmp:
            bare = Path(tmp) / "bare"
            bare.mkdir()
            hook = bare / "sql-code-preflight.sh"
...
class PluginCopies(unittest.TestCase):
    def test_hook_copies_match_canonical(self):
        for name in ("sql-code-guard.sh", "sql-code-preflight.sh"):
            with self.subTest(name):
