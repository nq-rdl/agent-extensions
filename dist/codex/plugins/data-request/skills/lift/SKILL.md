...
name: lift
license: CC-BY-4.0
description: 'Close out request pipelines: inspect the pinned library, classify lift
  candidates with the human, file reusable work on the owning library, and record
...
Read `${PLUGIN_ROOT}/skills/guardrails/SKILL.md` and the shared
ledger contract at `${PLUGIN_ROOT}/skills/setup/references/lifts.rst` (canonical source:
...
Resolve the helper at `${PLUGIN_ROOT}/skills/setup/scripts/sqlreview.sh`.
...
via the host user-question tool, at most four per batch (use smaller batches if the host limit
