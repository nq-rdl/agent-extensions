...
description: Scaffold a new GitHub spec-kit extension — the extension.yml manifest,
  ...
  adding a quality-gate/integration/hook to the Spec-Driven-Development workflow,
  or when the user runs $speckit-dev:create. spec-kit ships no `specify extension
  init`, so this fills the scaffolding gap. Distinct from Claude Code plugins/skills
  and from OpenCode extensions.
metadata:
