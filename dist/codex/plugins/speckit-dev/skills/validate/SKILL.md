...
description: Lint an existing GitHub spec-kit extension against the real manifest
  schema and validation rules, and report pass/fail per rule. Use when checking a
  spec-kit extension before install/publish, debugging a `specify extension add` rejection,
