#!/usr/bin/env bash
# PreToolUse guard (sql-review plugin), matcher Write|Edit. Fires only for paths under a
...
#   config.json                         ask   (config changes go through /sql-review:setup)
...
[ -n "$checker" ] || decide deny "SQL Review guard cannot find sqlreview.sh (looked under \${CLAUDE_PLUGIN_ROOT}/skills/setup/scripts/ and next to this hook). The plugin install is incomplete — reinstall sql-review@rdl-agent-extensions before writing $rel."
