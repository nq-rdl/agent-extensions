...
function normalizeReasoningEffort(effort) {
  ...
  if (!VALID_REASONING_EFFORTS.has(normalized)) {
    // Warn but pass through: the effort catalog is server-fetched and codex is the
    // authority on per-model gating (e.g. no `ultra` on Luna). Do not hard-reject.
    process.stderr.write(
      `Warning: unrecognized reasoning effort "${effort}". ` +
        `Known efforts: low, medium, high, xhigh, max, ultra. Passing through to codex.\n`
    );
  }
  return normalized;
}
