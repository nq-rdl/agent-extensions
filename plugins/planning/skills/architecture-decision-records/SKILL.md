...
- **Archival cue** — a `specs/NNN-slug/` spec has merged (the speckit `merge-spec`
...
- Next number = **max existing numeric prefix + 1**, scanning **both** `NNNN-*.md` and
  legacy `adr-NNNN-*.md`. Numbers are sequential and **never reused**, even after a
  deletion. Never rename or overwrite a pre-existing file.
...
4. Append exactly **one** row to `docs/adr/README.md`: `| NNNN | Title | Status | Date |`.
...
Read `docs/adr/README.md`, match the question against index titles, open the matched
file, and answer from its **Decision Drivers** + **Decision Outcome**. On no match (or
no store), say it is **not recorded** and offer to capture it — **never fabricate a
rationale**.
