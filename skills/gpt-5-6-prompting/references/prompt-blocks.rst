...
Reusable XML-tagged blocks for Codex or GPT-5.6 prompts. State each block once;
do not repeat it. Wrap each block in the XML tag shown in its heading.
...
One consolidated proactivity + approval-boundary block. 5.6 is proactive, so
give it a single policy here instead of scattering "ask first" lines through the
prompt (which causes over-asking).
...
.. code-block:: xml
    ...
    Do not re-confirm routine steps.
    </autonomy_policy>
