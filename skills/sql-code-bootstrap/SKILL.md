...
description: >-
  ...
  on an existing scope walks through what changed. Use at the start of the SQL Code scoping workflow,
  after /sql-code:setup and before /sql-code:analyse.
...
# SQL Code — bootstrap (Data Engineer)
...
Show the rendered `scope.md`. Use `/sql-code:map` for source mapping and
`/sql-code:draft` to develop the scoped SQL. Next stage, once the SQL exists: `/sql-code:analyse <sql path>`.
