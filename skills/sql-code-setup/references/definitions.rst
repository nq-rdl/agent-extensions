SQL Code — shared definitions
===============================

These definitions are the single source of truth for every ``/sql-code:*`` stage. They ship as
the ``definitions`` object in the bundled default ``assets/sqlreview/config.json`` and are copied
into the project's ``.sqlreview/config.json`` by ``/sql-code:setup``. Bootstrap, analyse and
explain read them from the project config at run time and never restate them; edit the project's
``config.json`` (via ``/sql-code:setup``) to change the wording for one project, or this file and
