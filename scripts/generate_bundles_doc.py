#!/usr/bin/env python3
"""Generate docs/bundles.md from the registry.

`docs/bundles.md` previously listed bundles by hand and rotted the moment the
registry changed (it still advertised the retired `swe`/`infra`/`meta` domain
bundles long after they were regrouped by subject). This is the same metadata-rot
failure mode as #100, so the fix is the same: generate the doc from the single
source of truth and gate it in CI.

The doc depends ONLY on `registry/`, never on upstream skill
frontmatter — so it changes only when a maintainer deliberately edits a bundle,
not on every cosmetic skills sync. That keeps the `--check` gate quiet.

CLI:
    python3 scripts/generate_bundles_doc.py [REPO_ROOT]            # write
    python3 scripts/generate_bundles_doc.py [REPO_ROOT] --check    # fail on drift
"""
from __future__ import annotations

import sys
from pathlib import Path

import yaml

from _registry import normalize_member

DOC_REL = Path("docs") / "bundles.md"


def _read_yaml(path: Path) -> dict:
    with path.open() as fh:
        return yaml.safe_load(fh) or {}


def _enabled_bundles(repo: Path) -> dict[str, dict]:
    out: dict[str, dict] = {}
    bundles_dir = repo / "registry" / "bundles"
    for bf in sorted(list(bundles_dir.glob("*.yaml")) + list(bundles_dir.glob("*.yml"))):
        data = _read_yaml(bf)
        if data.get("agents"):
            raise ValueError(f"{bf.name}: agents are retired; use skills with delegation references")
        claude = (data.get("targets") or {}).get("claude") or {}
        codex = (data.get("targets") or {}).get("codex") or {}
        if not claude.get("enabled"):
            continue
        plugin = claude.get("pluginName") or data.get("id") or bf.stem
        leaves = []
        for m in data.get("skills") or []:
            try:
                _src, leaf = normalize_member(m)
            except ValueError as exc:
                print(
                    f"::warning::Skipping malformed skill member {m!r} in {plugin!r}: {exc}",
                    file=sys.stderr,
                )
                continue
            leaves.append(leaf)
        out[plugin] = {
            "description": data.get("description") or "",
            "skills": leaves,
            "mcp": list(data.get("mcp") or []),
            "hooks": list(data.get("hooks") or []),
            "codex": bool(codex.get("enabled")),
            "codex_skills": [normalize_member(m)[1] for m in data.get("skills", []) if normalize_member(m)[0] not in codex.get("excludeSkills", [])] if (codex.get("components") or {}).get("skills", True) else [],
            "codex_mcp": bool((codex.get("components") or {}).get("mcp")),
            "codex_hooks": bool((codex.get("components") or {}).get("hooks")),
        }
    return out


def _ordered(order: list[str], enabled: dict) -> list[str]:
    ordered = [p for p in order if p in enabled]
    return ordered + sorted(set(enabled) - set(ordered))


def generate(repo) -> str:
    repo = Path(repo)
    mkt = _read_yaml(repo / "registry" / "marketplace.yaml")
    enabled = _enabled_bundles(repo)
    order = _ordered(list(mkt.get("order") or []), enabled)

    lines: list[str] = [
        "---",
        "icon: lucide/package",
        "---",
        "",
        "# Plugins",
        "",
        "<!-- GENERATED by scripts/generate_bundles_doc.py from registry/ — do not edit by hand. -->",
        "",
        "Each **subject** (a tool, library, language, or workflow) is one plugin.",
        "Claude Code and Codex publish separate self-contained packages for every bundle.",
        "Codex packages carry strict skill names and explicitly selected native capabilities.",
        "Delegation outlines are optional skill references, not registered agent types.",
        "",
        "## At a glance",
        "",
        "| Plugin | Claude Code | Codex | Description |",
        "|---|:---:|:---:|---|",
    ]
    for plugin in order:
        desc = enabled[plugin]["description"].rstrip(".").replace("|", "\\|")
        codex = "Yes" if enabled[plugin]["codex"] else "-"
        lines.append(f"| [`{plugin}`](#{plugin}) | Yes | {codex} | {desc} |")
    lines += [
        "",
        "## Install",
        "",
        "### Claude Code",
        "",
        "Add the marketplace, then install the subject plugin(s) you need:",
        "",
        "```bash",
        "/plugin marketplace add nq-rdl/agent-extensions",
        f"/plugin install <subject>@{mkt['name']}",
        "```",
        "",
        "New to the team? Install `rdl-team` first for onboarding, setup, and config-management workflows:",
        "",
        "```bash",
        f"/plugin install rdl-team@{mkt['name']}",
        "```",
        "",
        "### Codex",
        "",
        "Add this repository as a marketplace, then install one of the Codex-enabled subjects:",
        "",
        "```bash",
        "codex plugin marketplace add nq-rdl/agent-extensions",
        f"codex plugin add <subject>@{mkt['name']}",
        "```",
        "",
        "See [Codex support](codex.md) for verification commands, native hook coverage, and directory readiness.",
        "",
    ]

    for plugin in order:
        b = enabled[plugin]
        lines += ["---", "", f"## {plugin}", ""]
        desc = b["description"].rstrip(".")
        if desc:
            lines += [desc + ".", ""]
        if b["skills"]:
            lines.append("**Claude Code skills**")
            lines.append("")
            for leaf in b["skills"]:
                lines.append(f"- `/{plugin}:{leaf}`")
            lines.append("")
            if b["codex"]:
                lines.append("**Codex skills**")
                lines.append("")
                for leaf in b["codex_skills"]:
                    lines.append(f"- `${plugin}:{leaf}`")
                lines.append("")
        if b["codex_mcp"]:
            lines += ["**Codex MCP:** " + ", ".join(b["mcp"]), ""]
        if b["codex_hooks"]:
            lines += ["**Codex hooks:** Native command hooks; see [coverage and limitations](codex.md#hooks).", ""]
        if b["mcp"]:
            lines.append(
                f"**Claude Code MCP server(s):** {', '.join('`' + m + '`' for m in b['mcp'])}"
            )
            lines.append("")
        if b["hooks"]:
            lines.append(
                f"**Claude Code hooks:** {', '.join('`' + h + '`' for h in b['hooks'])}"
            )
            lines.append("")

    return "\n".join(lines).rstrip() + "\n"


def main(argv=None) -> int:
    argv = list(sys.argv[1:] if argv is None else argv)
    do_check = "--check" in argv
    positional = [a for a in argv if not a.startswith("--")]
    repo = Path(positional[0]) if positional else Path(".")
    target = repo / DOC_REL
    expected = generate(repo)
    if do_check:
        actual = target.read_text(encoding="utf-8") if target.is_file() else None
        if actual != expected:
            reason = "missing" if actual is None else "stale"
            print(
                f"::error::{DOC_REL} is {reason} — run "
                "`python3 scripts/generate_bundles_doc.py .`",
                file=sys.stderr,
            )
            return 1
        print(f"{DOC_REL} is in sync.")
        return 0
    target.write_text(expected, encoding="utf-8")
    print(f"  ✓ {DOC_REL}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
