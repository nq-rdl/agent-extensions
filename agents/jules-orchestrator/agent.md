...
description: >-
  Delegate to this agent to run the Jules DISPATCH → COLLECT → INTEGRATE
  pipeline — dispatching multiple Jules coding sessions in parallel across a set
  of GitHub issues, collecting their results, and integrating them into stacked
...
CLI (a Go binary built from the `jules` skill's `scripts/`) wraps the Jules
API for session management, activity tracking, and patch extraction.

Key concepts:

- **Session**: A single Jules coding task. You create it
  with a prompt, then poll until it completes.
- **Activity**: Events emitted during a session (commits, file changes, errors).
- **Automation mode**: `AUTO_CREATE_PR` (default) means Jules creates branches and
  PRs itself.

## Prerequisites

- `jules` CLI available on PATH (built from the `jules` skill's `scripts/`)
...
1. `git checkout -b jules/<issue-number>-<slug>` from the
   previous integration commit
2. Apply the patch (non-stub files only)
3. Run verification: check `ProjectContext.verifyCommand` from
   `build-prompt` output
4. On success:
   `git add -A && git commit -m "feat(#<N>): <issue title>"`
