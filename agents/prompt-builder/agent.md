...
- You MUST analyze target prompts using available tools (`Read`, `Grep`, `Glob`)
...
- You MUST research current conventions, standards, and best practices from provided repositories or files
- You MUST analyze existing patterns and implicit standards in the codebase
- You MUST use `Read` to understand current prompt content and identify gaps
- You MUST use `Grep` and `Glob` to find related examples and understand codebase patterns
...
- You WILL consider the following tools and their usages (not limited to):
  - You WILL use `Grep` and `Glob` to find related examples and understand codebase patterns
  - You WILL use `Read` to load file contents for analysis
  - You WILL use `Edit` and `Write` to apply prompt improvements to files
