#!/usr/bin/env bash
# SessionStart hook (sql-code plugin): one declarative line of context about the session's
# project — whether .sqlreview/ is initialised and which reviews are stale or missing — so the
# model knows which /sql-code:* stage applies before it is asked. Runs `sqlreview.sh status
...
if [ "$rc" -eq 3 ]; then
  ...
  [ -n "$has_sql" ] || exit 0
  ctx="SQL Review: this project contains SQL files but has no .sqlreview/ directory. /sql-code:setup initialises it (once per project); then /sql-code:bootstrap, :analyse and :explain use it."
  ...
  ctx="$(printf '%s' "$status" | jq -r '
    ...
    (.counts.total // 0) as $n
    | if $n == 0 then
        "SQL Review: .sqlreview/ is initialised (schema \(.schemaVersion)) with no reviews yet — /sql-code:bootstrap scopes a new SQL file, /sql-code:analyse reviews an existing one."
      else
        "SQL Review: .sqlreview/ is initialised (schema \(.schemaVersion)), \($n) review\(if $n == 1 then "" else "s" end): "
        + ([ (.counts.draft // 0 | select(. > 0) | "\(.) drafts (resume /sql-code:bootstrap or :analyse: \(names("draft")))"),
        ...
        + ". Stale or baseline-less reviews need /sql-code:analyse --update before /sql-code:explain; a missing SQL file is rebound with sqlreview.sh move."
      end' 2>/dev/null)"
fi
[ -n "$ctx" ] || exit 0
