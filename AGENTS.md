...
- **Refresh plugin trees** by running `bash scripts/sync-plugins.sh` (or pass a bundle name to scope it). The script reads `registry/bundles/<b>.yaml`, removes any stale copies, and rewrites `plugins/<b>/skills/<name>/` and `plugins/<b>/agents/<name>.md` from the canonical sources. It also rewrites this repo's own live `.claude/scripts/{install-deps,announce-capabilities}.sh` cc-web-setup SessionStart hook copies from canonical `skills/cc-web-setup/assets/` (preserving the exec bit, since those copies are invoked directly from `.claude/settings.json`), and drift-checks them (bytes + exec mode) under `--check`; the repo-local `.claude/scripts/install-deps.local.sh` seam has no canonical source and is left untouched.
