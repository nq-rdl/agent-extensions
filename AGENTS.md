...
skills and agents (canonical content under `skills/` and `agents/`) and publishes self-contained
...
This repo is a multi-target agent extension catalog. It maintains a single source of truth for reusable agent skills and agents, then publishes target-specific manifests over self-contained `plugins/<bundle>/` trees. Canonical content lives once under `skills/` and `agents/`.
...
```bash
# Validate all Claude/Codex plugin manifests, hooks, skills, and agents
