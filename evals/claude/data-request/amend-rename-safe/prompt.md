---
description: "Renaming an already-surfaced output column is analyst-safe: rename= in .select(), record it, check rows and keys"
...
The researcher wants the `MRN` column called `URN` in the extract. Nothing else changes.
Classify the amendment, then show me the source change you would make, the entry you would
add to the amendment record, and how you would check it before we re-release.
