...
`.claude/settings.json` declares them under `enabledPlugins` with their sources under `extraKnownMarketplaces`. **Important (verified live, 2.1.185): the platform registers the `extraKnownMarketplaces` at session start but does NOT install the `enabledPlugins` from them** — `enabledPlugins` only *enables* an already-installed plugin. So this repo installs the declared set itself, via `.claude/scripts/install-deps.sh` (`ensure_plugins`, reading the set straight from `settings.json`). There are two layers:

- **Setup-script field (first session).** Set the web environment's **Setup script** field to `CLAUDE_CODE_REMOTE=true make install-deps`. It runs *before* Claude starts, so the plugins are pre-seeded into the snapshot **before skill enumeration** — the only way their `/<plugin>:<skill>` commands appear on the **first** session. (This is one manual, per-environment UI step that cannot be committed.)
- **SessionStart hook (self-heal + resumes).** `install-deps.sh --session` runs every session (gated on `CLAUDE_CODE_REMOTE`); it re-installs anything missing. Because Claude enumerates skills at startup *before* hooks run, a hook-installed plugin surfaces from the **next** session — so the hook covers resumes, the Setup-script covers session 1.

`announce-capabilities.sh` cross-checks the declared set against `claude plugin list` and flags any **"Declared but NOT installed"** plugin, so a failed install is never silently masked.

**Local dev:** run `make install-deps` to provision the dev toolchain (lefthook, changie, gopls, python/jq). Without `CLAUDE_CODE_REMOTE=true` it skips the web runtime (gh/codex/plugin pre-seed) — a contributor uses their own gh/plugins.

To add a dev-helper plugin: set it `true` in `enabledPlugins` and register its marketplace under `extraKnownMarketplaces`. Keep the set small and **external** — do not enable the `rdl` marketplace or the `rdl@rdl` meta-plugin here (it is self-referential in this repo's own dev env and breaks the install batch).
